...
class PinMapper {
   ...
   */
    getDriver(name) {
      if (!this.drivers[name]) {
        throw new Error("Could not find PCA9685 driver by name "+name+" from PinMapper. Did you initialize it properly?")
     }
   ...
   */
    getDriverByAddress(address) {
      ...
      throw new Error("Could not find PCA9685 driver by address "+address+" from PinMapper. Did you initialize it properly?");
    }

    /**
     * configure what pins get what order
     * @param mapping {object} array with Pin Mappings
     */
   ...
   */
    ...
    setBrightness(mappedPin, brightness)
    {
      ...
      if(isNaN(brightness)) {
        console.warn(`Not setting ${mappedPin} to ${brightness}`);
        return this;
      }
